...
void DLinkedList<T>::add(int index, T e) {
    ...
    if (index < count / 2) {
        aft = head->next;
        for (int i = 0; i < index; i++) aft = aft->next;
    }
    else {
        aft = tail;
        for (int i = count; i > index; i--) aft = aft->prev;
    }
    Node *node = new Node(e, aft, aft->prev);
    aft->prev->next = node;
    aft->prev = node;
...
T DLinkedList<T>::removeAt(int index){
    ...
    if (index < count / 2) {
        ...
        p = tail->prev;
        for (int i = count - 1; i > index; i--) p = p->prev;
    }
    p->prev->next = p->next;
    p->next->prev = p->prev;
...
int  DLinkedList<T>::indexOf(T item){
    //YOUR CODE HERE
    int i = 0; Node *p = head->next;
    while (p != tail) {
        if (equals(p->data,item,itemEqual)) return i;
        ++i, p = p->next;
    }
    return -1;
}
